...
class Wheels:
    ...
    def calc_and_send_motor_power(self, x, y, r):
        ...
        pid_proccessed_list = [] * len(converted_to_rpm_list)
        for i in range(len(converted_to_rpm_list)):
            pid_proccessed_list[i] = (self.__feedback_object[i].pid_controll(converted_to_rpm_list[i], None)) #FIXME: CANから取得するコード書け
            
                
        recalcued_to_robomas_command_list = list(map(lambda rps: rpm_2_robomas_command(rad_per_sec_2_rpm(rps)), pid_proccessed_list))
...
def test():
    ...
    three_omni.calc_and_send_motor_power(x=0, y=0, r=1)
